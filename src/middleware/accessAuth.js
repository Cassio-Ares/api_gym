...
export const authIntructor = (req, res, next) => {
  try {
    ...
    if (!isValid) {
      return res
        .status(401)
        .json({ message: "Acesso negado nenhum token invalido" });
    }
...
export const authCollaboratorOrInstructor = (req, res, next) => {
  try {
    const token = req.token;
    ...
    if (!token) {
      return res
        .status(401)
        .json({ message: "Acesso negado nenhum tokne fornecido." });
    }

    let isValid = verifyTokenTeam(token) || verifyTokenInstructor(token);
